...
api/leaderboard.mjs    # GET: 20 המובילים בכל טבלה, עם הלוק של כל אחד, והמקום שלך
...
- `nm:<שם מנורמל>`: מי מחזיק בשם (`SET NX`), ו-`nmof`: איזה שם מחזיק כל שחקן. שם מנורמל = בלי רווחים ובאותיות קטנות, כך ש"נהוראי המלך" ו"נהוראיהמלך" הם אותו שם. כשמחליפים שם, הקודם משתחרר.
- `looks`: הלוק האחרון של הנהוראי של כל שחקן, לפודיום ולדמויות ליד השורות. השרת שומר רק משבצות מוכרות עם מזהים קצרים, והמשחק מצייר רק פריטים שקיימים בקטלוג (`safeLook` ב-`ui/board.js`).
...
- כל שם הוא "נהוראי" ועוד עד 10 תווים משל השחקן, בלי תווים נסתרים. מילים פוגעניות נמחקות. מירוץ בלי תוספת לשם (`nameRequired`) או בשם של שחקן אחר (`nameTaken`) לא נכנס לטבלה.
