...
// Events from testing (localhost, ?dev, automated browsers like the Playwright tests) are sent too, marked is_test:true,
// so charts can filter them out. Every event is also kept in window.__amp, so the tests can check it.
...
const isTest=/^(localhost|127\.0\.0\.1)$/.test(location.hostname)||new URLSearchParams(location.search).has('dev')||!!navigator.webdriver;
...
function track(type,props){
  ...
  const ev={event_type:type,device_id:playerId()||'no-storage',session_id:session,time:now,insert_id:`${session}-${n++}`,platform:'Web',language:navigator.language,
    event_properties:{...props,is_test:isTest},user_properties:{$set:{is_test:isTest,device,career_points:Stats.career,races:Stats.races,wins:Stats.wins}}};
  log.push(ev);
