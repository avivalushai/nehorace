...
// asking player's own rank. Player ids are never sent back; rows only carry a name and a "me" flag.
...
export async function GET(req){
  ...
  const names=ids.length?(await pipeline([['HMGET',KEYS.names,...ids]]))[0]:[];
  const nameOf=Object.fromEntries(ids.map((m,i)=>[m,names[i]||'נהוראי']));
  const rows=list=>list.map(([m,s],i)=>({rank:i+1,name:nameOf[m],score:s,me:m===me}));
